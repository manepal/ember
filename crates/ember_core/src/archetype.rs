...
impl Drop for Column {
    fn drop(&mut self) {
        ...
        // Only drop instances if the T type implements Drop explicitly.
        if let Some(drop_fn) = self.item_drop_fn {
...
impl Column {
    ...
    pub fn clone_empty(&self) -> Self {
        let data = if self.item_layout.size() == 0 {
             ptr::NonNull::new(self.item_layout.align() as *mut u8).unwrap().as_ptr()
        } else {
    ...
    pub unsafe fn grow(&mut self, new_capacity: usize) {
        ...
        let new_data = if self.capacity == 0 {
            alloc(Layout::from_size_align_unchecked(new_size, self.item_layout.align()))
        } else {
            realloc(self.data, old_layout, new_size)
        };

        if new_data.is_null() {
            std::alloc::handle_alloc_error(Layout::from_size_align_unchecked(new_size, self.item_layout.align()));
        }
...
impl Archetype {
        ...
        for column in &mut self.columns {
            if index >= column.capacity {
                unsafe {
                    ...
                    let new_cap = if column.capacity == 0 { 4 } else { column.capacity * 2 };
                    column.grow(new_cap);
                }
            }
        }
    ...
    pub unsafe fn insert_component<T: Component>(&mut self, row: usize, component: T) {
        if let Some(&col_idx) = self.component_indices.get(&ComponentId::of::<T>()) {
            ...
            panic!("Attempted to insert component {:?} into an Archetype that does not define it.", std::any::type_name::<T>());
        }
    }
    ...
    pub unsafe fn migrate_entity_to(&mut self, entity: Entity, target: &mut Archetype) -> Option<(Entity, usize)> {
...
mod tests {
    ...
    fn archetype_migration_moves_memory_and_shifts_tail() {
        ...
        unsafe {
            ...
            let a_idx = *source.component_indices.get(&ComponentId::of::<ComponentA>()).unwrap();
            let b_idx = *source.component_indices.get(&ComponentId::of::<ComponentB>()).unwrap();
